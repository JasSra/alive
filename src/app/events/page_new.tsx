...
export default function EventsPage() {
  ...
  const { events: liveEvents, status } = useLiveFeed<LiveSSEData>(live, transport, 1000);
  ...
  const fetchRange = async () => {
    ...
    try {
      const data = (await getRangeEvents({ from, to, userScope: "all", limit: 2000 })) as EventEntry[];
